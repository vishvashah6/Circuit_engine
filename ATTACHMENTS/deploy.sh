...
export FUNCTION_NAME="CIRCUITS_ENGINE_DOCUMENT_ATTACHMENT"
export HANDLER=uploader.handler
...
MD5=`cat uploader.js | md5sum | cut -d' ' -f1`
...
then
    ...
    zip -rq -X uploader.zip .
else
    cd build
fi
export PACKAGE=uploader.zip
