...
export FUNCTION_NAME="CIRCUITS_ENGINE_DYNAMO_TRIGGER"
export HANDLER=circuits_trigger.handler
export DESCRIPTION='Updates Dynamo information in Elastic Search'
...
MD5=`cat circuits_trigger.js | md5sum | cut -d' ' -f1`
...
then
    ...
    zip -rq -X circuits_trigger.zip .
else
    cd build
fi
export PACKAGE=circuits_trigger.zip
