...
export FUNCTION_NAME="CIRCUITS_ENGINE_CIRCUITS_ADD_COMMENT"
export HANDLER=comments.handler
export DESCRIPTION='Add comments(s)'
...
MD5=`cat comments.js | md5sum | cut -d' ' -f1`
...
then
    ...
    zip -rq -X comments.zip .
else
    cd build
fi
export PACKAGE=comments.zip
