...
export FUNCTION_NAME="CIRCUITS_ENGINE_QUERY_DOCUMENT_ATTACHMENT"
export HANDLER=downloader.handler
export DESCRIPTION='Attach document(s)'
...
MD5=`cat downloader.js | md5sum | cut -d' ' -f1`
...
then
    ...
    zip -rq -X downloader.zip .
else
    cd build
fi
export PACKAGE=downloader.zip
